...
cd $wrkdir
#
# Check to see if the executable file exists, if not make it.
#
if (! -f $exedir/$surf.cab) then
    if (! -f $potdir/$surf.o) then
        cd $potdir
        $CF $surf.f
    endif
    if (! -f $potdir/cab.o) then
        cd $potdir
        $CF cab.f
    endif
    cd $wrkdir
    $LD $exedir/$surf.cab $potdir/$surf.o $potdir/cab.o $abcdir/*.o 
endif
#
cp $ofile.dat abc.5
#
