...
module Brep
  ...
  integer(kind=i4b),dimension(:),allocatable :: nr_z,nphi_z,ntheta_z,zone_flag
  integer(kind=i4b) :: n,p,p_Patch,z,phi_sectors
...
module Open_information
  ...
  integer(kind=i4b) :: max_open,max_index,alpha,beta,counter,rad_elem,lmax,long_max,location,nE
  ...
  complex(kind=dpc), dimension(:,:,:), allocatable :: S_eigenvec
  integer(kind=i4b) :: max_l=11
  save max_open
  save max_index
  !-------------------------------------------------------------------------
...
module Calc_func
  ...
  real(kind=dbl) :: Energy_Hara,polar,polar_2,r_cutoff,Energy_fast_bound
  !-------------------------------------------------------------------------
  ...
  ! Energy_fast_bound = Energy of the last bound state of the molecule (input to Hara potential)
