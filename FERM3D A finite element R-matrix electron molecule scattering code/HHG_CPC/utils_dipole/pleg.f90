...
x=cos(theta)
if ((l-1).ge.m) then
der_plg=-sin(theta)*(l*x*plgndr(l,m,x)-(l+m)*plgndr(l-1,m,x))/(x*x-1)
else
der_plg=-sin(theta)*(l*x*plgndr(l,m,x))/(x*x-1)
end if
  fact1=factrl(l-m)
  fact2=factrl(l+m)
 norm=sqrt((2*l+1)*fact1/(Pi*fact2))
...
130   CGAMMA = ZEXP(SUM)
      RETURN
900   FORMAT(1X,2E14.7,10X,49H ARGUMENT OF GAMMA FUNCTION IS TOO CLOSE TO&
     & A POLE)
