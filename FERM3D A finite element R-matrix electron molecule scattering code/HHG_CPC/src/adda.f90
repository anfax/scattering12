...
!  use control, only : cont
  implicit none
  logical :: cont=.TRUE.

  real(kind=dbl) :: E_ryd,radZ
  real(kind=dbl),parameter ::ac=1.d-14
...
E_ryd=2.0d0*Ene/charge**2
l=int(real(lambda))
radZ=rad*charge
call coulomb(cont,l,E_ryd,radZ,ac,fc_ret,gc_ret,fcp_ret,gcp_ret)
