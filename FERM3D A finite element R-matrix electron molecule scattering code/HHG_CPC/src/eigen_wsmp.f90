...
SUBROUTINE eigensparse_wsmp(Hmat,Omat,rowind,colptr,nbasis,nnz,shift, &
  ...
  INTEGER :: colptr(0:nbasis), rowind(nnz),mtype
  REAL(kind=8) :: shift
  ...
  INTEGER, ALLOCATABLE :: ipiv(:), rowind_save(:),colptr_save(:)
  ...
  REAL(kind=8), ALLOCATABLE :: workl(:),rhs(:,:)
  REAL(kind=8) :: tol, norm
  CHARACTER :: bmat*1, which*2, save_fact
  LOGICAL, ALLOCATABLE :: select(:)
  LOGICAL :: rvec 
  !write(6,*)'begin eigen_pardiso'
  ...
  ALLOCATE( Shftmat(nnz), rowind_save(nnz), colptr_save(0:nbasis) )
  allocate(rhs(nbasis,1))
  rowind_save(:) = rowind(:)
  colptr_save(:) = colptr(:)
  Shftmat(:) = Hmat(:) - shift*Omat(:)
  ! tol used as dummy variable, not used
  save_fact = 'Y'
  mtype=11 ! Asymmetric Matrix
  nrhs = 0
  ...
  DO itercount = 1, mxitercount
     ...
     IF (ido == -1) THEN
        ...
        !           | workd(ipntr(2)).                          |
        !           %-------------------------------------------%
        !write(6,*)'inside eigen_pardiso',nnz,nrhs,nbasis
        ...
        rhs(:,1)=workd(ipntr(2):ipntr(2)+nbasis-1)
        !call pardiso_sym(Shftmat,rowind,Colptr(0),nnz,nbasis,nrhs,rhs,ierr,mtype)
        call master(Shftmat,rowind,Colptr(0),nnz,nbasis,nrhs,rhs,0)
        ierr=0
        workd(ipntr(2):ipntr(2)+nbasis-1)=rhs(:,1)
        !write(6,*)'after 1st system solve'
        rowind(:) = rowind_save(:)
        colptr(:) = colptr_save(:)
  ...
  RETURN
END SUBROUTINE eigensparse_wsmp
