...
!  end interface
  ...
  ! Sparse solver setup, store sparse matrices in format useful for Pardiso
  ! (symmetric compressed row)
  !---------------------------
           ...
           do i=1,ni
              if ((gamma_c(j,i).ne.0).and.(bionda2_boundary(gamma_c(j,i)).ne.'up_fun')) then
                 ...
                 if ((dabs(gamma_v(j,i)).ge.tolerance).or.(dabs(area_overlap_v(j,i)).ge.tolerance)) then
                    if (Row.ge.Col) then ! USE THIS IF STATEMENT TO EXPLOIT SYMMETRY OF THE MATRICES 
                       nnz=nnz+1
