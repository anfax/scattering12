...
subroutine call_coulomb_complex(Ene,lambda,charge,rad,fc_ret,gc_ret,fcp_ret,gcp_ret)
  !  program call_coulomb_complex
  use nrtype, only : dbl,dpc,i4b
  implicit none
  real(kind=dbl) :: epsi,zero,half,one,four,Pi,rerr
  real(kind=dbl),intent(in) :: Ene,charge,rad
  complex(kind=dpc),intent(in) :: lambda
  COMPLEX(kind=dpc) :: X,ETA,ZLMIN,FC(201),GC(201),FCP(201),GCP(201), &       
       & SIG(201),ZL,WS,CI,CGAMMA
  real(kind=dbl),intent(out) :: fc_ret,gc_ret,fcp_ret,gcp_ret                        
  INTEGER(kind=i4b) LDISP(8),nl,mode,kfn,ifail,md,ih,kfin,ii,jj, &
...
10 continue
  ...
  do ii=0,nl-1
     ...
     fcp(ii+1)=fcp(ii+1)*((-2.d0*eta)**(zlmin+ii+1)*exp(Pi*eta/2.d0) /(2.d0** &
          &  (zlmin+ii)*abs(cgamma(zlmin+ii+1.d0+eta*cmplx(0.d0,1.d0))))) / (1.d0/sqrt(2.d0*ene))
     gcp(ii+1)=gcp(ii+1)*2.d0**(zlmin+ii)*abs(cgamma(zlmin+ii+1.d0+eta*cmplx(0.d0,1.d0)))&
          &*exp(-Pi* eta/2.d0)/(Pi*(-2.d0*eta)**(zlmin+ii)) / (1.d0/sqrt(2.d0*ene))
...
end subroutine call_coulomb_complex
